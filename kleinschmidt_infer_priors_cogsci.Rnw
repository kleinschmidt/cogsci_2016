...
others]{Bertelson2003,Clarke2004,Kraljic2005,Norris2003}.
...
previous work, we use an independent, conjugate Normal-$\chi^2$ prior for each
category, with parameters \cite{Gelman2003}
\begin{eqnarray}
  \label{eq:2}
...
prior_summary %>% 
  transmute(Parameter = format_params(stat),
            Expected = mean,
            `95\\% HPD Interval` = sprintf('%.0f--%.0f', low, high)) %>%
  as.data.frame() %>%
...
\cite{Lisker1964}.  The distribution for /b/ deviates from prior work,
...
This is a possibility that requires further study. First, large-scale corpus
studies of VOT distributions would be informative about the true underlying
cross-talker variability in these distributions, but the only study of this type
we are aware of considers only non-negative VOTs \cite{Chodroff2015} and cannot
detect talkers who prevoice their /b/.  Second, more modeling work is needed to
test whether a multimodal prior is justified given the adaptation data, and if
so, whether it would change the inference about listeners' prior expectations
for /b/.
...
as if they had already observed around 200--800 samples (for the category means
and variances, respectively) from the unfamiliar talker they encountered in our
experiemnt.  This makes sense, given that there is not as much variability
across talkers in VOT distributions \cite{Allen2003,Chodroff2015}.  As a result,
past experience with other talkers' VOT distributions is, in principle, highly
informative about the VOT distributions that an unfamiliar talker will produce,
requiring less adaptation.  This leads naturally to the prediction that
listeners will be less confident in their prior beliefs for cues and categories
where there \emph{is} substantial variability across talkers, like spectral cues
to fricatives \cite{McMurray2011a,Newman2001} or vowels
\cite{Hillenbrand1995,Peterson1952}.
