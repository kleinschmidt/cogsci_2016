...
% TOOD: kind of liked the old version of this better. reconcile.
Critically, this statistical inference process draws on implicit 
...
produced by the unfamiliar talker themselves. Specifically, a listener's experience with other 
...
produced a distribution of cues for the /b/ and /p/ sounds that had
substantially lower means than a typical talker.  
...
could avoid the need for costly and time-consuming annotation of production data from many
...
\caption{Percentage of boundary shift from typical talker to each exposure
  ...
  prior beliefs. Typical and exposure talker boundaries were too close together to
...
\section{Inferring prior beliefs from adpatation} %% [TFJ: how about ``Inferring priors about inter-talker variability'' or something like that?]
\label{sec:model}
...
pseudocounts (which is essentialy uniform on the whole range from completely ignoring prior beliefs to
not adapting at all).
...
an \emph{ideal adapter} which combines prior beliefs with current experience
...
the range of adaptation behavior observed across the various accents that listeners
