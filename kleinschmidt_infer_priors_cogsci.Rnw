...
Such adaptation can be understood as a form of statistical inference. This insight
is captured by a recent proposal, the {\em ideal adapter} framework \cite{Kleinschmidt2015}. 
Each talker's particular accent (way of talking) can be formalized as the distribution of
acoustic cues that they produce for each phonetic category (or other underlying 
linguistic unit). Listeners are taken to adapt to an unfamiliar talker via 
\emph{distributional learning}, inferring the underlying talker-specific cue 
distributions from the talker's productions.

Critically, this statistical inference process is taken to draw on implicit 
beliefs about {\em how} talkers tend to differ from each other. As a 
consequence, adaptation  to an unfamiliar talker should depend on a listener's 
prior experience with other talkers, rather than only on the speech 
produced by the unfamiliar talker themselves. Specifically, a listener's experience with others 
talkers provides the starting point for the distributional learning required for
adaptation, or, in Bayesian terms, a \emph{prior belief} about the 
probability of different possible accents (cue distributions). 
More informative prior beliefs can substantially reduce the amount of direct evidence 
needed to converge on accurate beliefs about the current talker's cue distributions.

The goals of the present work are two-fold. First, we test a critical prediction of 
the ideal adapter framework.  To the extent
...
to adapt, leading to slowed or incomplete adaptation.  There is some 
evidence that this is the case.  For instance, \citeA{Idemaru2011} found that
listeners have difficulty adapting to a talker who produces anti-correlated
...
substantially lower [TFJ:less variance? lower mean?] than a typical talker.  

However, no studies have systematically probed whether and how a listener's
prior expectations constrain phonetic adaptation.  To that end, we expose listeners
to a range of different accents, which differ (only) in the cue distributions for /b/
...
the novel talker's cue distributions, depending on the {\em a priori} typicality 
...
suggests that listeners not only have beliefs about the {\em average} distribution
of cues (as suggested in previous work, [TFJ: REF]), but also have implicit 
beliefs about the ways in which talkers tend to differ from each other. This 
emphasized the second question we seek to address: what is the nature of these 
beliefs about inter-talker variability? We use the behavioral data 
obtained in our experiment to \emph{infer} listeners' underlying prior beliefs 
about the way in which talkers differ from each other (in the distributions of 
cues to /b/ and /p/ that they produce). Previous work has used similar data to 
infer prior beliefs about {\em average} cue distributions (e.g., to /b/ and /p/), 
in essence assuming stationarity of these distributions across talkers [TFJ: REF]. 
In investigating in what ways listeners believe these distributions to vary across talkers, 
we aim to shed light on the mechanisms and representations that afford robust 
speech perception despite inter-talker variability in the realization of phonetic
categories. [TFJ: could be shortened; just trying to make the point] By aiming to
infer these priors from {\em perception} data, we explore an approach that in the future 
could avoids the need for costly and time-consuming annotation of production data from many
different talkers. This would provide a powerful method for investigating listeners'
prior beliefs that could be applied to other cues, categories, and
...
implicitly corresponding to /b/ and /p/, respectively.  [TFJ: say that VOT is primary cue to /p/ vs. /b/ in english?]
This distribution
...
[TFJ: the table is confusing. what does -28\% mean? especially since the -10ms condition then has a positive percentage. 
It hink I know what you mean, but it's confusing. how about just writing "n.s." into that row? and then explaining in the 
text?]
...
boundaries %>% 
  ...
  transmute(`/b/ mean VOT` = bvotCond,
            `Mean shift` = sprintf('%d\\%%', observed),
            `95\\% CI` = sprintf('%d--%d\\%%', ci_lo, ci_high)) %>%
...
\section{Model}[TFJ: how about ``Inferring priors about inter-talker variability'' or something like that?]
\label{sec:model}

Our second goal in this paper is to test whether it is possible to infer
listeners prior beliefs 
...
\cite{Kleinschmidt2012c}. Previous modeling
work along these lines [TFJ: REF] has treated the content of listeners prior beliefs---the
...
cluster, and finding a compromise between these observed statistics and their
...
never adapting at all). [TFJ: can you be clearer here: 888 prior pseudocounts would seem to 
large preempt adaptation]
The prior for the lapsing rate was uniform on $[0,1]$.
...
null model of \Sexpr{round(mod_goodness_of_fit[['LL_ratio']])}, [TFJ: p less than 0.001, I assume based on model comparison; also provide p-value for correlation]
and Spearman's
...
[TFJ: wait, doesn't it just mean that the mean across subjects is reasonably well modeled this way? 
I then realized what you meant and edited this a bit]
...
speakers produce pre-voiced /b/ \cite{Lisker1964,Docherty2011}, which is
...
underlying cross-talker variability in these distributions [TFJ: be more specific?], but the only study
...
At first blush, this conflicts with previous 
work on another phonetic contrast, /b/-/d/, which found confidence values that 
were one or two orders of magnitude smaller than those inferred here {Kleinschmidt2015}. 
Interestingly, the /b/-/d/ contrast is cued by spectral cues (formant frequency transitions) which generally vary
...
in the current study and in \citeA{Kleinschmidt2015} is actually in line with
an ``ideal adapter'' which combines prior beliefs with current experience
weighted according to confidence.  This idea finds
...
/b/-/p/ contrast used here) much less than an /s/-\ph S contrast (where the latter exhibits larger
 variability across talkers; \citeNP{McMurray2011a, Newman2001}).
...
produced by a typical talker and the exposure talker.  Second, 
the range of adaptation behavior observed across the various accents listeners
heard can be captured by a belief-updating model with a single set of 
prior expectations that are updated based on
...
[TFJ: nice. a bit rough around the edges. you probably can cut some refs, too; but mostly I think it can be submitted as is]
